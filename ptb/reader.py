...
def _read_words(filename):
  ...
  with tf.gfile.GFile(filename, "r") as f:
    for line in f:
        ...
        sentence = [int(word) for word in sentence if word!='']
        if len(sentence) < max_document_length:
            temp = sentence
            for i in range(max_document_length - len(sentence)):
                temp.append(10001)
            sentence = temp
...
def blackholes_raw_data(data_path=None):
  ...
  train_path = os.path.join(data_path, "blackholes.train.txt")
  valid_path = os.path.join(data_path, "blackholes.valid.txt")
  
  #At the end we extract hidden states from entire dataset (train + validation)
  test_path = os.path.join(data_path, "blackholes.total.txt")
